 ...
 * @author DELL
 */
@WebServlet(name = "BestSellerServlet", urlPatterns = {"/bestseller"})
public class BestSellerServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            String url = HOME;
            int id = FoodDAO.getBestSeller();
            Food f = FoodDAO.getFoodByID(String.valueOf(id));
            ArrayList<Category> c = CategoryDAO.getAllCategory();
            request.setAttribute("category", c);
            if(f.getIsdelete() == 0){
                request.setAttribute("best", f);
            }
